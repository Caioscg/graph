...
void dfs(struct Graph* graph, int node, int max_depth, int* edge_count, int visited[MAX_NODES]) {
    visited[node] = 1;

    if (max_depth > 0) {
        struct Node* near = graph->adjacency_list[node];
        while (near != NULL) {
            if (!visited[near->value]) {
                (*edge_count)++;
                dfs(graph, near->value, max_depth - 1, edge_count, visited);
            }
            near = near->next;
        }
    }
